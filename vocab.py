...
class Vocab(object):
    ...
    """
    def load_pretrained_word_embeddings(self, embedding_path, kernel='default'):
        trained_embeddings = {}
        ...
        print("=====unk token id {} , pad token id {}".format(self.word2id[self.unk_token], self.word2id[self.pad_token]))

        if self.unk_token not in self.word2id:
            print("====unknown token is not in word2id=====", self.size())
        # load embeddings
        self.word_embeddings = np.random.uniform(low=-0.1, high=0.1, size=(self.word_size(), self.word_embed_dim))
